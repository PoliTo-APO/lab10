...
def main():
    ...
    # ottengo calcolo valori nutritivi tramite lambda
    print("{:.1f}".format(r1.calculate_nutritional_value(lambda x: x.calories)))
    print("{:.1f}".format(r1.calculate_nutritional_value(lambda x: x.proteins)))
    print("{:.1f}".format(r1.calculate_nutritional_value(lambda x: x.carbs)))
    print("{:.1f}".format(r1.calculate_nutritional_value(lambda x: x.fats)))
    ...
    print("---------- R5 ----------")
    food.create_recipe("Lasagna")
    food.create_recipe("Arrosto di Maiale")
    food.create_recipe("Trofie al Pesto")
    food.create_recipe("Patate al forno")
    food.create_recipe("Flan di Verdure")
    food.create_recipe("Vitello tonnato")
    food.create_recipe("Frittata")

    food.add_chef("Paolo", "Flan di Verdure", ["Lasagna", "Arrosto di Maiale"])
    food.add_chef("Simone", "Trofie al Pesto", ["Flan di Verdure"])
    food.add_chef("Antonio", "Arrosto di Maiale", ["Trofie al Pesto", "Patate al forno"])
    food.add_chef("Vittorio", "Lasagna")

    food.add_chef("Luca", "Vitello tonnato")
